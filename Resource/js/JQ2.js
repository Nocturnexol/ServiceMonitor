...
var JQ = {
    InitTable: function (n) {
        ...
        t.jqGrid({
            ...
            height: n.height ? n.height : $(window).height() - 60-74-50+'px',
        ...
        $(window).bind("resize", function () {
            var x = t.closest(".jqGrid_wrapper").width();
            t.setGridWidth(x);
            if (n.height == undefined || n.height=="")
               t.setGridHeight($(window).height() - 60 - 74 - 50);
            var jqWidth = t.width();
            var jqparWidth = t.parent().parent().width();
            if (jqWidth >= jqparWidth - 1 && jqWidth <= jqparWidth + 1) {
                t.parent().parent().css("overflow-x", "hidden");
            }
        });
        ...
        for (var i = 0; i < col.length; i++) {
            ...
            if (item.formatter != undefined) {
                if (item.formatter == "date") {
                    ...
                    Model.formatter = "date";
                    Model.formatoptions = { srcformat: 'Y-m-d H:i:s', newformat: 'Y-m-d H:i:s' };
                }
