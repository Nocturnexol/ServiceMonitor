...
!function(a) {
    ...
    function b(a, b) {
        window.console && window.console.trace && (console.trace(a),
        b && console.trace(b))
    }
    function c(a) {
        return a.data()
    ...
    function d(a, b, c) {
        if (b && b.key) {
            var d, e = c.separator || ",";
            c && c.multiWord ? (d = a.val().split(e),
            d[d.length - 1] = b.key,
            a.val(d.join(e)).focus()) : a.attr("data-id", b.id).focus().val(b.key),
    ...
    function e(a, b, c) {
        if (b.is(":visible")) {
            ...
            v && !c.showBtn && (b.parents(".input-group").hasClass("dropup") ? (d.top = "auto",
            d.bottom = a.parent().height()) : (d.top = a.parent().height(),
            d.bottom = "auto")),
    ...
    function f(a, b) {
        var c, d, e;
        return b.indexId === -1 && !b.idField || b.multiWord ? a : (c = a.css("background-color").replace(/ /g, "").split(",", 3).join(","),
        d = b.inputBgColor || "rgba(255,255,255,0.1)",
        e = b.inputWarnColor || "rgba(255,255,0,0.1)",
        a.attr("data-id") || !a.val() ? a.css("background", d) : (~e.indexOf(c) || a.trigger("onUnsetSelectValue").css("background", e),
        a))
    }
    function g(a, b, c) {
        var d, e, f = a.parent().find("tbody tr." + c.listHoverCSS);
        f.length && (d = (f.index() + 3) * f.height(),
        e = Number(b.css("max-height").replace("px", "")),
        d > e || b.scrollTop() > e ? d -= e : d = 0,
        b.scrollTop(d))
    }
    function h(a, b) {
    ...
    function m(a, b, c) {
        var d, f, g, h, i, j, l = a.parent().find("ul.dropdown-menu"), m = 0, n = ['<table class="table table-condensed table-sm" style="margin:0">'];
        if (!b || !(d = b.value.length))
            return l.empty().hide(),
            a;
        var o = b.value;
        if (c._lastData && JSON.stringify(c._lastData.value) === JSON.stringify(o) && l.find("tr").length === d)
            return l.show(),
            e(a, l, c),
            a;
        if (c._lastData = b,
        c.showHeader) {
            n.push("<thead><tr>");
            for (g in o[0])
                k(g, c) && (n.push("<th>", c.effectiveFieldsAlias[g] || g, 0 === m ? "(" + d + ")" : "", "</th>"),
                m++);
            n.push("</tr></thead>")
        }
        n.push("<tbody>");
        ...
        setTimeout(function() {
            if (!t) {
                var a = l.find("table:eq(0)")
                  , b = 0
                  , c = 0;
                l.height() < a.height() && Number(l.css("min-width").replace("px", "")) < l.width() && (b = 18,
                c = 20),
    ...
    function o(b, c, d, e) {
        return d = a.trim(d),
        e.ignorecase && (b = b.toLocaleLowerCase(),
    ...
    function p(b, c, d, e) {
        ...
        if (b = b || "",
        a.isFunction(e.fnPreprocessKeyword) && (b = e.fnPreprocessKeyword(b, e)),
        e.url)
            n(e, b).done(function(a) {
                d(c, e.data, e),
            ...
            g = j(f))
                if (b) {
                    for (k = f.value.length,
                    h = 0; h < k; h++)
                        for (i in f.value[h])
                            if (f.value[h][i] && o(b, i, f.value[h][i] + "", e)) {
    ...
    var s, t, u = a(window), v = "ActiveXObject"in window, w = navigator.userAgent.match(/Chrome\/(\d+)/);
    w && (w = Number(w[1])),
    t = v || w > 51;
    var x = {
        url: null,
      ...
      , y = {
        init: function(b) {
            ...
            j.each(function() {
                var j, k, l = a(this), n = r(l, b), o = l.parents(".input-group").find("ul.dropdown-menu:eq(0)");
                return i(l, o, b) ? (b.showBtn || l.css("border-radius", "4px").parents(".input-group:eq(0)").css("width", "100%").find(".btn:eq(0)").hide(),
                l.removeClass("disabled").prop("disabled", !1).attr("autocomplete", "off"),
                o.css(b.listStyle),
                b.inputBgColor || (b.inputBgColor = l.css("background-color")),
                l.on("keydown", function(a) {
                    var e, f;
                    if (o.is(":visible")) {
                        if (e = o.find("." + b.listHoverCSS),
                        f = "",
                        h(o, b),
                        a.keyCode === b.keyDown) {
                            if (e.length ? e.next().length ? f = c(e.next().mouseover()) : b.autoSelect && l.val(l.attr("alt")).attr("data-id", "") : f = c(o.find("tbody tr:first").mouseover()),
                            g(l, o, b),
                            !b.autoSelect)
                                return
                        } else if (a.keyCode === b.keyUp) {
                            if (e.length ? e.prev().length ? f = c(e.prev().mouseover()) : b.autoSelect && l.val(l.attr("alt")).attr("data-id", "") : f = c(o.find("tbody tr:last").mouseover()),
                            g(l, o, b),
                            !b.autoSelect)
                                return
                        } else
                            a.keyCode === b.keyEnter ? (f = c(e),
                            o.hide().empty()) : l.attr("data-id", "");
                        d(l, f, b)
                    }
                }).on("compositionstart", function(a) {
                    s = !0
                }).on("compositionend", function(a) {
                    s = !1
                }).on("keyup input paste", function(c) {
                    var d;
                    return ~a.inArray(c.keyCode, [b.keyDown, b.keyUp, b.keyEnter]) ? (l.val(l.val()),
                    void f(l, b)) : (c.keyCode && f(l, b),
                    clearTimeout(k),
                    void (k = setTimeout(function() {
                        s || (d = l.val(),
                        a.trim(d) && d === l.attr("alt") || (l.attr("alt", d),
                        b.multiWord && (d = d.split(b.separator).reverse()[0]),
                        (d.length || b.allowNoKeyword) && b.fnGetData(a.trim(d), l, m, b)))
                    }, b.delay || 300)))
                }).on("focus", function() {
                    e(l, o, b)
                }).on("blur", function() {
                    ...
                    var c = l.val();
                    return a.trim(c) && c === l.attr("alt") && o.find("table tr").length ? o.show() : void (o.is(":visible") || (b.multiWord && (c = c.split(b.separator).reverse()[0]),
                    (c.length || b.allowNoKeyword) && b.fnGetData(a.trim(c), l, m, b)))
                }),
                l.parent().find(".btn:eq(0)").attr("data-toggle", "").click(function() {
                    var a = "none";
                    return o.css("display") === a && (a = "block",
                    b.url ? (l.click().focus(),
                    o.find("tr").length || (a = "none")) : m(l, b.data, b)),
                    o.css("display", a),
                    !1
                }),
