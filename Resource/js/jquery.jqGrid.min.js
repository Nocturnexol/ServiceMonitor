	    ...
	    return this.each(function () {
	        if (!this.grid) {
				...
				L = function (a, b, c, e) {
					...
					d.p.localReader.id = h), d.p.reccount = 0, $.isXMLDoc(a)) {
...
-1 !== d.p.treeANode || d.p.scroll ? b = b > 1 ? b : 1 : (H.call(d, !1, !0), b = 1);
				...
				R = function (a) {
				    if (!d.grid.hDiv.loading) {
				        ...
				        //  $.isFunction(d.p.condition) && d.p.condition.call();
				        ...
				        switch (b = d.p.datatype.toLowerCase()) {
				            ...
				            case "xml":
				            case "script":
				                if (d.p.condition != null && $.isArray(d.p.condition) && d.p.condition.length > 0) {
				                    var conditions = d.p.condition;
				                    var cond = '';
				                    for (var ii = 0; ii < conditions.length; ii++) {
				                        if ($(conditions[ii].value) != undefined && $(conditions[ii].value).val() != null && $(conditions[ii].value).val() != "") {
				                            cond += "¡¾" + conditions[ii].key + ":" + $(conditions[ii].value).val() + "¡¿";
				                        }
				                    }
				                    $('.ui-jqgrid .ui-jqgrid-condition').text(cond);
				                }
				                $.ajax($.extend({
				                    url:
								d.p.url,
				                    ...
				                    success: function (c, f, g) {
				                        if (c.summary != null && $.isArray(c.summary) && c.summary.length > 0) {
				                            var summary = c.summary;
				                            var sum = '';
				                            for (var ii = 0; ii < summary.length; ii++) {
				                                if (summary[ii] != null && summary[ii].name != null && summary[ii].value != undefined && summary[ii].value != "") {
				                                    sum += "¡¾" + summary[ii].name + ":" + summary[ii].value + "¡¿";
				                                }
				                            }
				                            $('.ui-jqgrid .ui-jqgrid-summary').text(sum);
				                        }
	            ...
	            var Db = d.p.hidegrid === !0 ? $("<a role='link' class='ui-jqgrid-titlebar-close HeaderButton " + p + "' title='' />").hover(function () {
	                Db.addClass(o)
	            },
				function () {
				    Db.removeClass(o)
				}).append("<span class='ui-jqgrid-headlink " + Bb + "'></span>").css("rtl" === h ? "left" : "right", "0px") : "";
	            if ($(e.cDiv).append(Db).append("<span class='ui-jqgrid-title'>" + d.p.caption + "</span><span class='ui-jqgrid-summary'></span><span class='ui-jqgrid-condition'></span>").addClass("ui-jqgrid-titlebar ui-jqgrid-caption" + ("rtl" === h ? "-rtl" : "") + " " + k(l, "gridtitleBox", !0)), $(e.cDiv).insertBefore(e.hDiv), d.p.toolbar[0]) {
